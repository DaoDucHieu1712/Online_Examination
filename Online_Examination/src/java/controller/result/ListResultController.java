 ...
 * @author ADMIN
 */
public class ListResultController extends BaseRequireAuthentication {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (name_search.length() == 0) {
            list_result = result_dao.getAllResultAndPaging(pageIndex, pageSize);
            totalResult = result_dao.countResult();
        } else {
            list_result = result_dao.getAllResultByNameSearchAndPaging(name_search, pageIndex, pageSize);
            totalResult = result_dao.countResultByNameSearch(name_search);
        }
        ...
        request.getRequestDispatcher("../view/result/list.jsp").forward(request, response);
